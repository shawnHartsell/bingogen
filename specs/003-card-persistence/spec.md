...
- Q: Should card deletion require confirmation? → A: Yes, a confirmation prompt is required before deleting (irreversible once deleted from the database).

## Assumptions

- Persistence uses MongoDB (local instance for development). Cards are stored as full JSON documents and updated via whole-document replacement. Hosted infrastructure (Atlas, containers, Terraform) is deferred to a later feature.
- Each user manages their own cards; there is no multi-user or authentication model.
- Card identifiers are generated by MongoDB (ObjectId) on the server and are guaranteed unique.
